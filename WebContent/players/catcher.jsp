		...
		function playersTB() {
			...
			$.ajax({ // 선수 정보 저장되어 있는 json file 연결
				...
				success: function(doc) {
					list = doc.catcher // json 중 key가 catcher인 jsonArray
						...
						for (var i = 0; i < list.length; i+=3) { // 3*n 형태의 테이블이기 때문에 인덱스 3씩 증가
							...
							if((list.length - i) < 3){ // 마지막행, 출력할 선수가 3명 미만 남은 경우 남은 선수의 수만큼 for문 실행
								for (var j = 0; j < (list.length - i); j++) {
									...
									// 테이블 만들어서 total에 누적
									total += '<td class="onePlayer"><table class="fixTable">'
									+ '<tr height="40"><td width="160"><p class="number">'
									+ number + '</p></td><td class="picPlayer" rowspan="4">'
									+ '<div class="picPlayer"><img src=' + img
									+ ' style="height: 260px; position: relative; left: -50px;">'
									+ '</div></td></tr><tr height="50"><td><h1 class="name">' + name
